            ...
            <x-splade-table :for="$table" striped>
                <x-splade-cell actions>
                    <div class="flex justify-start">
                        <Link href="/admin/notifications-templates/{{ $item->id }}/send" class="px-2 text-green-500" confirm>
                            <div class="flex justify-start space-x-2">
                                <x-heroicon-s-paper-airplane class="h-4 w-4 ltr:mr-2 rtl:ml-2"/>
                                <span>{{trans('tomato-notifications::global.templates.send')}}</span>
                            </div>
                        </Link>
                        <Link href="/admin/notifications-templates/{{ $item->id }}" class="px-2 text-blue-500" modal>
                            <div class="flex justify-start space-x-2">
                                <x-heroicon-s-eye class="h-4 w-4 ltr:mr-2 rtl:ml-2"/>
                                <span>{{trans('tomato-admin::global.crud.view')}}</span>
                            </div>
                        </Link>
                        <Link href="/admin/notifications-templates/{{ $item->id }}/edit" class="px-2 text-yellow-400" modal>
                            <div class="flex justify-start space-x-2">
                                <x-heroicon-s-pencil class="h-4 w-4 ltr:mr-2 rtl:ml-2"/>
                                <span>{{trans('tomato-admin::global.crud.edit')}}</span>
                            </div>
                        </Link>
                        <Link href="/admin/notifications-templates/{{ $item->id }}"
